...
- 凭据由 Windows 加密保存在当前用户本机；新安装不含开发者的账号、私人 API 或目录设置。旧版专用服务保留独立入口，不自动迁移凭据和云端数据。
...
- Credentials are encrypted locally for the current Windows user. Fresh installations contain no developer account, private API connection or directory preferences. Existing legacy connections remain separate.
